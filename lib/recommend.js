...
export async function buildRecommendations(input) {
  ...
  if (!personA || !personB || !query || Number.isNaN(parsedTime.valueOf())) {
    throw new Error("Locations, request, and meeting time are required");
  }
  if (
  ...
  const origins = await Promise.all([
    geocode(personA, apiKey),
    geocode(personB, apiKey),
  ]);
  const center = {
