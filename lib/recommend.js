...
async function chooseWithGemini({ recommendations, participants, weather, query }) {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey || !recommendations.length) return null;
  const people = (participants || []).map((participant, index) => ({
    person: index + 1,
    statedPreference: clean(participant.preference),
    savedPreferences: participant.profile?.preferences || {},
    timelineSummary: participant.profile?.timelineSummary
      ? {
          visitCount: participant.profile.timelineSummary.visitCount,
          topPlaces: participant.profile.timelineSummary.topPlaces,
          activityTypes: participant.profile.timelineSummary.activityTypes,
        }
      : null,
  }));
  const candidates = recommendations.map((place) => ({
    id: place.id,
    name: place.name,
    address: place.address,
    rating: place.rating,
    priceLevel: place.priceLevel,
    journeys: place.journeys,
    mapsScore: place.score,
    mapsReason: place.explanation,
  }));
  const prompt = `You are MeetSpot's final group decision maker. Pick exactly one candidate.
Prioritise hard dietary/accessibility needs, then group preference overlap, fair travel,
weather suitability, ratings, and a useful balance between familiar tastes and novelty.
Do not invent venue facts. Exact origin locations, emails, and raw Timeline data are not included.

Group request: ${query}
Weather: ${JSON.stringify(weather)}
Participant summaries: ${JSON.stringify(people)}
Verified candidate data: ${JSON.stringify(candidates)}`;
    ...
    "https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent",
    {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          ...
          responseSchema: {
            type: "OBJECT",
            required: ["selectedPlaceId", "headline", "reason", "tradeoff"],
            properties: {
              selectedPlaceId: { type: "STRING", enum: candidates.map((place) => place.id) },
  ...
  const selected = recommendations.find((place) => place.id === decision.selectedPlaceId);
  if (!selected) throw new Error("Gemini selected an unknown place");
  return { ...decision, selectedPlace: selected.name, model: "gemini-3.6-flash" };
}
...
export async function buildRecommendations(input) {
  ...
  let aiDecision = null;
  try {
    aiDecision = await chooseWithGemini({
      recommendations,
    ...
    console.error("Gemini final decision failed:", error.message);
  }
  return { origins, weather, recommendations, aiDecision, maxMinutes, travelMode };
}
